# -*- coding: utf-8 -*-
"""Research pass 9 (truly final): red songs + Laufer series completeness."""
...
results = {}
for label, q in [
    ('redsongs', 'filetype:audio 游击队歌 OR 大刀进行曲 OR 毕业歌 OR 松花江上'),
    ('yulong-series', 'intitle:"Yu lung hsi f"'),
    ('tahsiang-series', 'intitle:"Ta hsiang shan"'),
    ('huqin', 'filetype:audio huqin OR 弦子 OR 中国音乐'),
    ('revolution-china', 'filetype:audio chinese revolution song'),
]:
    results[label] = commons_search(label, q)

json.dump(results, open(OUT + 'research9.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print('SAVED research9.json')
